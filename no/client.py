...
def periodomassimo(i, per):
	...
	if not inc == []:
		p = [((e[0], e[1]), e[1] - e[0]) for e in inc]
		ord = sorted(p,	key = itemgetter(1), reverse = True)
		return(ord[0][0])
	else:
		return(i)
...
giocate = []
periodoprec = None
p = [(periodomassimo(i[0], periodi), i[1:]) for i in partite]
ord = sorted(p, key = itemgetter(0))
for i in ord:
	periodo = i[0]
#	print(periodo)
	if periodoprec == None:
...
with open('giocatori.json', 'w', encoding='utf8') as file:
	json.dump(giocatori, file, sort_keys = True, indent = "\t", ensure_ascii = False)
